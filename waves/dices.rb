...
let RATEMULT = 10000
let RATE1  = 39655 # 3.9655 x RATE_MULT
let RATE2  = 24600 # 2.4600 x RATE_MULT
let RATE3  = 19000 # 1.9000 x RATE_MULT
let RATE4  = 14200 # 1.4200 x RATE_MULT
let RATE5  = 11400 # 1.1400 x RATE_MULT

let RATES       = [RATE1, RATE2,  RATE3,  RATE4,  RATE5]
let BETS        = [BET1,  BET2,   BET4,   BET8,   BET14]
...
func validateBetAndGetWinAmount(bet: Int, internalAssetIdx: Int, playerChoice: String) = {
    ...
    if dicesCount < 1 || dicesCount > 5 then throw("Invalid dices count in player's choice") else

    (bet * RATES[dicesCount - 1]) / RATEMULT
}
...
func generateRandChoise(gameId: String, rsaSign: ByteVector) = {
    ...
    let rand = toInt(sha256(rsaSign)) % 6
    let result = if rand < 0 then -1 * rand else rand
...
# "WAVES:${wavesDecimals}:${wavesBetDivider}:${reservedAmount}_${assetId}:${assetDecimals}:${assetBetDivider}:${reservedAmount}_..."
@Callable(i)
func constructorV1(rsaPublic64: String, benzAddress: String, randOracleTimeFrame: Int, tokensDescriptor: String) = {
  ...
  func splitter(acc: (List[String], List[String], List[String], List[String]), elem: String) = {
  ...
  [StringEntry(RSAPUBLIC64KEY, rsaPublic64),
    ...
    StringEntry(allowedAssetsKey, makeString_2C(r._1, "_")),
    StringEntry(assetsDecimalsKey, makeString(r._2, "_")),
    StringEntry(betDividersKey, makeString(r._3, "_")),
    StringEntry(reservedAmountsKey, makeString_2C(r._4, ":")) # keys usually contain "_", so we use ":"
  ]
}
...
func withdraw(gameId: String, rsaSign: ByteVector) = {
    ...
    let randChoise = if (winByTimeout) then playerChoice.take(1) else generateRandChoise(gameId, rsaSign)
